/* ============================================================
   ...
   Resilient loader: avoid running twice when script is included multiple times
   ============================================================ */
...
function updateUI() {
  ...
  const vatRate = getNumber("vatRate") / 100;
  const customMargin = getNumber("customMargin");
  // Safe DOM setters to avoid runtime errors when parts of the UI are not present
  function setText(id, txt) {
    const el = document.getElementById(id);
    if (el) el.textContent = txt;
  }

  setText("batchLabel", costs.batchSize + " pcs");
  setText("unitCostLabel", formatCurrency(costs.baseCostPerUnit));
  setText("costMaterial", formatCurrency(costs.materialPerUnit));
  setText("costLabor", formatCurrency(costs.laborPerUnit));
  setText("costMachine", formatCurrency(costs.machinePerUnit));
  setText("costHardware", formatCurrency(costs.hardwarePerUnit));
  setText("costPackaging", formatCurrency(costs.packagingPerUnit));
  setText("costTotal", formatCurrency(costs.baseCostPerUnit));
  setText("customMarginLabel", customMargin + "%");
  ...
  pricingPresets.forEach((preset) => {
    ...
    if (preset.id === selectedPresetId) {
      setText("selectedPrice", formatCurrency(priceIncVat));
      setText("selectedMargin", margin + "%");
    }
  });
}
