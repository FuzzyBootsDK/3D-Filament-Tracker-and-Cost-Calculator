        ...
        foreach (var spool in filament.Spools)
            records.Add(new CsvFilamentRecord
            {
                ...
                PurchasePricePerKg = spool.PurchasePricePerKg.HasValue
                    ...
                    : filament.PricePerKg.HasValue
                        ? filament.PricePerKg.Value.ToString(CultureInfo.InvariantCulture)
                        : ""
            });
        ...
        foreach (var group in grouped)
        {
            ...
            foreach (var record in group)
                filament.Spools.Add(new Spool
                {
                    ...
                    PurchasePricePerKg = decimal.TryParse(record.PurchasePricePerKg, NumberStyles.Any,
...
public class CsvFilamentRecord
{
    ...
    [Name("Finish")] public string Finish { get; set; } = "";

    [Name("Color Name")] public string ColorName { get; set; } = "";

    [Name("Color Code")] public string ColorCode { get; set; } = "";

    [Name("Total Weight (g)")] public decimal TotalWeight { get; set; }

    [Name("Weight Remaining (g)")] public decimal WeightRemaining { get; set; }

    [Name("Quantity")] public int Quantity { get; set; }

    [Name("Spool Type")] public string SpoolType { get; set; } = "";

    [Name("Spool Material")] public string SpoolMaterial { get; set; } = "";

    [Name("Reusable Spool")] public string ReusableSpool { get; set; } = "";

    [Name("Diameter (mm)")] public decimal Diameter { get; set; }

    [Name("Location")] public string Location { get; set; } = "";

    [Name("Notes")] public string Notes { get; set; } = "";

    [Name("Date Added")] public string DateAdded { get; set; } = "";

    [Name("Purchase Price Per Kg")] public string PurchasePricePerKg { get; set; } = "";
}